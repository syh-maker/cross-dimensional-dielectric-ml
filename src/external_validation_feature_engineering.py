...
DEFAULT_CIF_DIR = "data/raw/external_validation"
DEFAULT_LABELS = "data/raw/external_validation/validation_labels_y.csv"
DEFAULT_OUTPUT_FEATS = "data/processed/external_validation/validation_features_X.csv"
DEFAULT_OUTPUT_LABELS = "data/processed/external_validation/validation_labels_y.csv"
...
def get_vacuum_agnostic_features(structure):
    ...
    try:
        ...
        top_elem = structure[top_idx].specie.symbol
        bottom_elem = structure[bottom_idx].specie.symbol

        r_top = get_vdw_radius(top_elem)
        r_bottom = get_vdw_radius(bottom_elem)
        ...
        nonmetal_atoms = sum(
            composition[element] for element in composition.elements if not element.is_metal
        )
        nonmetal_fraction = nonmetal_atoms / composition.num_atoms
        result["EDV"] = v_eff * nonmetal_fraction

        electronegativities = [
            Element(site.specie.symbol).X
            for site in structure
            if Element(site.specie.symbol).X is not None and Element(site.specie.symbol).X > 0
        ]
        ...
        top_en = Element(top_elem).X
        bottom_en = Element(bottom_elem).X
...
def load_structures(cif_dir, labels_path):
    labels_df = pd.read_csv(labels_path)
    required_columns = ["material_id", "e_electronic", "e_ionic", "e_total"]
    missing_columns = [
        column for column in required_columns if column not in labels_df.columns
    ]
    if missing_columns:
        raise ValueError(f"Missing label columns: {', '.join(missing_columns)}")

    structures = []
    for filename in labels_df["material_id"].astype(str):
    ...
    return structures, labels_df[required_columns].copy()
...
def main(args):
    print("Processing external validation set.")

    structures, labels_df = load_structures(args.cif_dir, args.labels)

    features_df = pd.DataFrame({"material_id": labels_df["material_id"]})
    ...
    valid_mask = features_df["t_eff"].notna()
    ...
    features_df.loc[valid_mask].to_csv(args.features_output, index=False)
    labels_df.loc[valid_mask].to_csv(args.labels_output, index=False)
...
def parse_args():
    ...
    parser.add_argument("--cif-dir", default=DEFAULT_CIF_DIR)
    parser.add_argument("--labels", default=DEFAULT_LABELS)
    parser.add_argument("--features-output", default=DEFAULT_OUTPUT_FEATS)
    parser.add_argument("--labels-output", default=DEFAULT_OUTPUT_LABELS)
